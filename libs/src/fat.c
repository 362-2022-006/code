...
bool find_fat_parameters(struct FATParameters *params, uint8_t sd_buffer[512]) {
...
bool _file_increment_sector(const struct FATParameters *params, struct FATFile *file) {
...
int get_file_next_sector(const struct FATParameters *params, struct FATFile *file,
                         uint8_t buffer[512]) {
    ...
    bool end = _file_increment_sector(params, file);
...
void open_root(const struct FATParameters *params, struct FATFile *file) {
...
void ls(const struct FATParameters *params, struct FATFile *file, uint8_t sd_buffer[512]) {
    ...
    while (!done && (length = get_file_next_sector(params, file, sd_buffer)) > 0) {
...
uint32_t _find_dir_entry_matching(const char *name, const struct FATParameters *params,
    ...
    while (!done && (length = get_file_next_sector(params, file, sd_buffer)) > 0) {
        ...
        sector = file_get_sector(params, file);
    }

    return 0;
}

bool open(const char *name, const struct FATParameters *params, struct FATFile *file,
          uint8_t sd_buffer[512]) {
    struct DIREntry *ent;

    if (_find_dir_entry_matching(name, params, file, sd_buffer, &ent)) {
...
bool set_size(const char *name, const uint32_t size, const struct FATParameters *params,
    ...
    struct DIREntry *ent;
    uint32_t sector;
    if ((sector = _find_dir_entry_matching(name, params, file, sd_buffer, &ent))) {
        printf("%p %p\n", sd_buffer, ent);
        ent->file_size = size;
...
uint32_t _get_open_cluster(const struct FATParameters *params, uint32_t previous_cluster,
                           uint8_t sd_buffer[512]) {
...
void new_file(const struct FATParameters *params, struct FATFile *file, const char *name,
    ...
    if (!done) {
        ...
        uint32_t sector =
            params->first_sector + params->reserved_sectors + file->next_cluster / 128;
        ...
        cluster = _get_open_cluster(params, 0, sd_buffer);
